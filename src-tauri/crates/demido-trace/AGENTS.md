...
| | A delegation's child is **`child` on the line**, never `agent`. A body is flattened onto the event, and the event's own `agent` is the parent that wrote it, so naming the child `agent` too wrote a line with one field twice: it serialised, and refused to read back. A log that had ever carried a delegation was an invalid session log, which is what the desk said in the one place it was ever going to be noticed ([#67](https://github.com/elpideus/demido-studio/issues/67)). The in-memory field keeps the name it deserves; the rename is at the wire, with a round-trip test each side of it. |
...
| | A call that opened a sub-agent is a `Moment::Delegated` rather than a `Moment::Called` ([#67](https://github.com/elpideus/demido-studio/issues/67)): one exchange, the task out and the answer back. Which calls those are is read off the `agent/delegated` events rather than off a tool name, so this projection knows nothing about what the registry calls its delegation tool. The task is the child's own first message and the answer is the child's own ending, both off the child's half of the log, which is why one rule covers both paths: at the default the call's result is what the child said, and above it the result is the paragraph saying the work has gone out. A delegation that opened no child, declined or past the depth, stays an ordinary call row with its stated reason, because nothing was delegated. |
...
of its own events, the scope that keeps a parent's transcript the parent's, and
the delegation the parent's transcript draws as one exchange.
