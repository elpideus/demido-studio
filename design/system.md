...
| Component | States | Tokens |
|---|---|---|
| Backend picker | available, unavailable, selected | Cards one step up from whatever island hosts them, which is `raised` on the wizard's `panel` page (`surfaces.md` rule 1, one step at a time); selected `edge`. A `--brand-*` mark at full strength only where the hardware is present, `ink-3` otherwise, keyed on the card that is in the machine rather than on whether a build is pinned for it; a row naming no vendor carries none. An unavailable row states why in `ink-3` and is never disabled. |
