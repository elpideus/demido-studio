...
| Delegation row | working, done, failed | `raised` with a violet accent (`surfaces.md` rule 4) and a bot icon, `rose` when the sub-agent's run ended badly. Carries the task that went out and the answer that came back, the answer in a `well` under the sub-agent's own name. A long answer folds by height at `--fold-answer`. |
...
| Component | States | Tokens |
|---|---|---|
| Backend picker | available, unavailable, selected | Cards one step up from whatever island hosts them, which is `raised` on the wizard's `panel` page (`surfaces.md` rule 1, one step at a time); selected `edge`. A `--brand-*` mark at full strength only where that vendor's card is in the machine, `ink-3` otherwise: keyed on the hardware, never on whether a build is pinned for it. Only the two rows that name a vendor carry one, because Vulkan is an API and CPU is the machine, and a mark that cannot honour "only where the hardware is present" does not belong on the row. An unavailable row states why in `ink-3` and is never disabled. |
...
| Wizard window | first launch, resumed, left | Its own window: the whole of it is `panel` at `--radius-island`, and the page is unfilled on that. Leaving is offered at every step and is never final (`setup.md` section 1). |
| Wizard step nav | current, done, later | A `chrome` bar across the top of the page; the steps are unfilled on it, `hover` under the pointer, current `edge`, done `ink-2`, later `ink-4`. A step nobody has settled carries a dot in `currentcolor`, not a tick, because a tick would claim it is done. |
| Runtime row | absent, ticked, managed, linked | A `panel` card on the page, the checkbox `signal`. States its download and its size on disk before a byte is spent; a row already on disk cannot be ticked, because a checkbox that cleared it would read as a delete. A linked row's bytes are never counted as disk Demido spent (`runtimes.md`). |
| Archive list | always | Unfilled under its runtime row, in the silkscreen voice: the name `ink-3` in mono, the sizes and the licence `ink-4`. A build and the runtime it links against are one row and two archives. |
| Fetch foot | idle, running, failed | The total for what is ticked at `ink-2`, then one button: `Fetch`, `Cancel` while it runs, `Try again` after a failure. The reason sits under it in `rose` and says what is still on disk, because a toast is gone before a download is. The progress bar is `signal` on a `well` track. A cancel is not a failure and draws none. |
| Model folder row | confirmed, suggested | `raised` on the page; a suggested folder quieter at `ink-3` until it is taken. Folders are read where they are, so nothing is moved and no symlink is made. |
| Desk set-up row | outstanding | A `panel` card under the transcript and above the composer, naming what is left and offering to take it up. Drawn from the plan, which is derived from disk, so a folder somebody deleted is offered again without anything being un-remembered. |
...
| Prompts page | listed, open, edited, suppressed, moved | A Settings page ([#58](https://github.com/elpideus/demido-studio/issues/58)), the paragraph register by id. A row opens in place and is `edge` while open, like the nav beside it; no card, because elevation would be standing in for state. An edited row marks its id `signal` and says `Edited`. Dependants sit above the field in `amber`; an edit **suppresses** a measured claim rather than being refused, and a suppressed one is struck at `ink-4`. Field and diff are `well`. A default that moved under an older edit carries the note and a diff in the monitor's own language: what this build ships lit on `edge`, what the edit has struck at `ink-4`. |
