...
| `cargo test --manifest-path src-tauri/Cargo.toml -p demido-chat --test a_real_model_with_tools -- --ignored --test-threads=1` | S2's: a model shown six tools reads a file somebody planted, greets without calling anything, is approved, is denied, meets a tool the picker switched off, and is offered a tool document edited between two questions. |
...
| `cargo test --manifest-path src-tauri/Cargo.toml -p demido-chat --test a_fetched_model -- --ignored --test-threads=1` | S3's: each tier's smallest quantisation fetched through the queue in the same run (about 25 GB, deleted by the next run), answering and calling a tool on all three tiers; the projector arriving with its weights, a borrowed model loading with its folder untouched, and the rebuild surviving a tool edit. Needs the network and the card. |
