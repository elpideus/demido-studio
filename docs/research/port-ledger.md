...
- **Drop**: none outright. demido-vram is the closest thing to a drop
  ...
  **Resolved on [#74](https://github.com/elpideus/demido-studio/issues/74):
  promoted, not deleted.** v3 did not port the `Ledger` or build the tiered
  scheduler. It ported the crate's refusal-before-the-driver idea into two
  consumers that each exist in the window: `demido_chat::Pool`, which asks
  `admit` how many generation slots open
  ([#65](https://github.com/elpideus/demido-studio/issues/65)), and the Models
  window's fit verdict, which prices a download as one slot through the same
  `admit` ([#74](https://github.com/elpideus/demido-studio/issues/74)). The
  crate's `AGENTS.md` records what was carried and what was reversed.
